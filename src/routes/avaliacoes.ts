...
export async function criarAvaliacao(c: Context<AppEnv>) {
  ...
  const { data, error } = await supabase
    .from('avaliacoes')
    .insert({ pagina_id: paginaId, usuario_id: userId, nota, comentario: body?.comentario ?? null })
    .select('id, pagina_id, nota, comentario, created_at')
    .single()
...
export async function minhasAvaliacoes(c: Context<AppEnv>) {
  ...
  const { data, error } = await supabase
    .from('avaliacoes')
    .select('id, pagina_id, nota, comentario, resposta, respondido_em, created_at')
